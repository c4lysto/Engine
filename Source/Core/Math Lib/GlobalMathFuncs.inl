...
FORCEINLINE Type Max(Type lhs, Type rhs)
{
	return (lhs > rhs) ? lhs : rhs;
}

template<> FORCEINLINE s8 Max(s8 lhs, s8 rhs) \
{ \
	s8 diff = lhs - rhs; \
	return lhs - (diff & (diff >> 7)); \
}

template<> FORCEINLINE s16 Max(s16 lhs, s16 rhs) \
{ \
	s16 diff = lhs - rhs; \
	return lhs - (diff & (diff >> 15)); \
}

template<> FORCEINLINE s32 Max(s32 lhs, s32 rhs) \
{ \
	s32 diff = lhs - rhs; \
	return lhs - (diff & (diff >> 31)); \
}

template<> FORCEINLINE s64 Max(s64 lhs, s64 rhs) \
{ \
	s64 diff = lhs - rhs; \
...
template<typename Type>
FORCEINLINE Type Min(Type lhs, Type rhs)
{
	return (lhs < rhs) ? lhs : rhs;
}

template<> FORCEINLINE s8 Min(s8 lhs, s8 rhs) \
{ \
	s8 diff = lhs - rhs; \
	return rhs + (diff & (diff >> 7)); \
}

template<> FORCEINLINE s16 Min(s16 lhs, s16 rhs) \
{ \
	s16 diff = lhs - rhs; \
	return rhs + (diff & (diff >> 15)); \
}

template<> FORCEINLINE s32 Min(s32 lhs, s32 rhs) \
{ \
	s32 diff = lhs - rhs; \
	return rhs + (diff & (diff >> 31)); \
}

template<> FORCEINLINE s64 Min(s64 lhs, s64 rhs) \
{ \
	s64 diff = lhs - rhs; \
...
template<typename Type>
FORCEINLINE Type Clamp(Type val, Type minVal, Type maxVal)
{
	return Max(Min(val, maxVal), minVal);
}

template<typename Type>
FORCEINLINE Type Saturate(Type val)
{
	return Clamp(val, Type(0.0f), Type(1.0f));
}

FORCEINLINE float Lerp(const float& lhs, const float& rhs, const float& fLambda)
{
	return lhs + (rhs - lhs) * fLambda;
}

FORCEINLINE double Lerp(const double& lhs, const double& rhs, const double& fLambda)
{
	return lhs + (rhs - lhs) * fLambda;
}

FORCEINLINE Vec2f_Out Lerp(Vec2f_In lhs, Vec2f_In rhs, const float& fLambda)
{
	return lhs + (rhs - lhs) * fLambda;
}

FORCEINLINE Vec3f_Out Lerp(Vec3f_In lhs, Vec3f_In rhs, const float& fLambda)
{
	return lhs + (rhs - lhs) * fLambda;
}

FORCEINLINE Vec4f_Out Lerp(Vec4f_In lhs, Vec4f_In rhs, const float& fLambda)
{
	return lhs + (rhs - lhs) * fLambda;
}

FORCEINLINE s8 Abs(const s8& iScalar)
{
	s8 s = iScalar >> 7;
	return (iScalar ^ s) - s;
}

FORCEINLINE s16 Abs(const s16& iScalar)
{
	s16 s = iScalar >> 15;
	return (iScalar ^ s) - s;
}

FORCEINLINE s32 Abs(const s32& iScalar)
{
	s32 s = iScalar >> 31;
	return (iScalar ^ s) - s;
}

FORCEINLINE s64 Abs(const s64& iScalar)
{
	s64 s = iScalar >> 63;
	return (iScalar ^ s) - s;
}

FORCEINLINE float Abs(const float& fScalar)
{
	return abs(fScalar);
}

FORCEINLINE double Abs(const double& fScalar)
{
	return abs(fScalar);
}

FORCEINLINE Vec2f_Out Abs(Vec2f_In vVector)
{
	return Vec2f(Abs(vVector.GetXRef()), Abs(vVector.GetYRef()));
}

FORCEINLINE Vec3f_Out Abs(Vec3f_In vVector)
{
	return Vec3f(Abs(vVector.GetXRef()), Abs(vVector.GetYRef()), Abs(vVector.GetZRef()));
}

FORCEINLINE Vec4f_Out Abs(Vec4f_In vVector)
{
	return Vec4f(Abs(vVector.GetXRef()), Abs(vVector.GetYRef()), Abs(vVector.GetZRef()), Abs(vVector.GetWRef()));
}

FORCEINLINE float Sqrt(const float& fScalar)
{
	return sqrtf(fScalar);
}

FORCEINLINE double Sqrt(const double& fScalar)
{
	return sqrt(fScalar);
}

FORCEINLINE Vec2f_Out Sqrt(Vec2f_In vVector)
{
	return Vec2f(Sqrt(vVector.GetXRef()), Sqrt(vVector.GetYRef()));
}

FORCEINLINE Vec3f_Out Sqrt(Vec3f_In vVector)
{
	return Vec3f(Sqrt(vVector.GetXRef()), Sqrt(vVector.GetYRef()), Sqrt(vVector.GetZRef()));
}

FORCEINLINE Vec4f_Out Sqrt(Vec4f_In vVector)
{
	return Vec4f(Sqrt(vVector.GetXRef()), Sqrt(vVector.GetYRef()), Sqrt(vVector.GetZRef()), Sqrt(vVector.GetWRef()));
}

FORCEINLINE float SqrtSafe(const float& fScalar, const float& safeVal /*= 0.0f*/)
{
	return fScalar != 0.0f ? Sqrt(fScalar) : safeVal;
}

FORCEINLINE double SqrtSafe(const double& fScalar, const double& safeVal /*= 0.0*/)
{
	return fScalar != 0.0 ? Sqrt(fScalar) : safeVal;
}

FORCEINLINE Vec2f_Out SqrtSafe(Vec2f_In vVector, Vec2f_In vSafeVec /*= Vec2f(I_ZERO)*/)
{
	return Vec2f(SqrtSafe(vVector.GetXRef(), vSafeVec.GetXRef()), \
				 SqrtSafe(vVector.GetYRef(), vSafeVec.GetYRef()));
}

FORCEINLINE Vec3f_Out SqrtSafe(Vec3f_In vVector, Vec3f_In vSafeVec /*= Vec3f(I_ZERO)*/)
{
...
FORCEINLINE Vec4f_Out SqrtSafe(Vec4f_In vVector, Vec4f_In vSafeVec /*= Vec4f(I_ZERO)*/)
{
...
FORCEINLINE Vec2f_Out Splat(Vec2f_In lhs)
{
	return Permute<splat, splat>(lhs);
}

template<VecElem splat>
FORCEINLINE Vec3f_Out Splat(Vec3f_In lhs)
{
	return Permute<splat, splat, splat>(lhs);
}

template<VecElem splat>
FORCEINLINE Vec4f_Out Splat(Vec4f_In lhs)
{
	return Permute<splat, splat, splat, splat>(lhs);
}

template<VecElem pX, VecElem pY>
FORCEINLINE Vec2f_Out Permute(Vec2f_In lhs)
{
...
FORCEINLINE Vec3f_Out Permute(Vec3f_In lhs)
{
...
FORCEINLINE Vec4f_Out Permute(Vec4f_In lhs)
{
...
template<VecElem pX, VecElem pY>
FORCEINLINE Vec2f_Out Permute(Vec2f_In lhs, Vec2f_In rhs)
{
...
template<VecElem pX, VecElem pY, VecElem pZ>
FORCEINLINE Vec3f_Out Permute(Vec3f_In lhs, Vec3f_In rhs)
{
...
template<VecElem pX, VecElem pY, VecElem pZ, VecElem pW> 
FORCEINLINE Vec4f_Out Permute(Vec4f_In lhs, Vec4f_In rhs)
{
...
FORCEINLINE s32 SignMask(Vec4f_In vVector)
{
	return s32(((vVector.GetXiRef() & BIT31) << (s32)VecElem::X) | \
			   ((vVector.GetYiRef() & BIT31) << (s32)VecElem::Y) | \
			   ((vVector.GetZiRef() & BIT31) << (s32)VecElem::Z) | \
...
FORCEINLINE Vec2f_Out SelectTF(Vec2f_In lhs, Vec2f_In rhs, Vec2f_In condition)
{
	return Vec2f((condition.GetXiRef() & BIT31) ? lhs.GetXRef() : rhs.GetXRef(),
				 (condition.GetYiRef() & BIT31) ? lhs.GetYRef() : rhs.GetYRef());
}

FORCEINLINE Vec3f_Out SelectTF(Vec3f_In lhs, Vec3f_In rhs, Vec3f_In condition)
{
...
FORCEINLINE Vec4f_Out SelectTF(Vec4f_In lhs, Vec4f_In rhs, Vec4f_In condition)
{
...
FORCEINLINE Vec2f_Out SinCos(const float& fRadians)
{
...
#define VEC_CMP_DEFBASE(name, varType, cmpDef)	\
	FORCEINLINE VecCmpResult name(varType##_In lhs, varType##_In rhs) \
{ \
	return VecCmpResult(cmpDef); \
}

#define VEC_CMP_DEF(name, amt, nameExt, varType) \
	FORCEINLINE bool name##nameExt(varType##_In lhs, varType##_In rhs) \
{ \
...
#define VEC_ARITH_DEF(name, varType, operations) \
	FORCEINLINE varType##_Out name(varType##_In lhs, varType##_In rhs) \
{ \
...
FORCEINLINE float Dot(Vec2f_In vVectorA, Vec2f_In vVectorB)
{
	return (vVectorA.x * vVectorB.x) + (vVectorA.y * vVectorB.y);
}

FORCEINLINE float Dot(Vec3f_In vVectorA, Vec3f_In vVectorB)
{
	return (vVectorA.x * vVectorB.x) + (vVectorA.y * vVectorB.y) + (vVectorA.z * vVectorB.z);
}

FORCEINLINE float Dot(Vec4f_In vVectorA, Vec4f_In vVectorB)
{
#if SSE_AVAILABLE
...
FORCEINLINE float Mag(Vec2f_In vVector)
{
	return sqrtf(MagSq(vVector));
}

FORCEINLINE float Mag(Vec3f_In vVector)
{
	return sqrtf(MagSq(vVector));
}

FORCEINLINE float Mag(Vec4f_In vVector)
{
#if SSE_AVAILABLE
...
FORCEINLINE float Length(Vec2f_In vVector)
{
	return Mag(vVector);
}

FORCEINLINE float Length(Vec3f_In vVector)
{
	return Mag(vVector);
}

FORCEINLINE float Length(Vec4f_In vVector)
{
	return Mag(vVector);
}

FORCEINLINE float MagSq(Vec2f_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE float MagSq(Vec3f_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE float MagSq(Vec4f_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE float LengthSq(Vec2f_In vVector)
{
	return MagSq(vVector);
}

FORCEINLINE float LengthSq(Vec3f_In vVector)
{
	return MagSq(vVector);
}

FORCEINLINE float LengthSq(Vec4f_In vVector)
{
	return MagSq(vVector);
}

FORCEINLINE Vec2f_Out Normalize(Vec2f_In vVector)
{
...
FORCEINLINE Vec3f_Out Normalize(Vec3f_In vVector)
{
	float mag = Mag(vVector);
...
FORCEINLINE Vec4f_Out Normalize(Vec4f_In vVector)
{
	float mag = Mag(vVector);
	if(mag)
	{
...
FORCEINLINE Mat33f_Out Normalize(Mat33f_In mMatrix)
{
	Mat33f tmp(mMatrix);
	tmp.Normalize();
	return tmp;
...
FORCEINLINE Mat44f_Out Normalize(Mat44f_In mMatrix)
{
	Mat44f tmp(mMatrix);
	tmp.Normalize();
	return tmp;
...
FORCEINLINE Vec4f_Out IntToFloat(Vec4f_In vVector)
{
...
FORCEINLINE Vec3f_Out Cross(Vec3f_In lhs, Vec3f_In rhs)
{
...
FORCEINLINE Mat44f Mat34ToMat44(Mat34f_In mMatrix)
{
...
FORCEINLINE Mat34f Mat44ToMat34(Mat44f_In mMatrix)
{
...
FORCEINLINE Mat33f_Out Transpose(Mat33f_In mMatrix)
{
...
FORCEINLINE Mat44f_Out Transpose(Mat44f_In mMatrix)
{
...
FORCEINLINE Mat44f_Out Transpose3x3(Mat44f_In mMatrix)
{
	Mat44f retVal(mMatrix);
	retVal.Transpose3x3();
	return retVal;
...
FORCEINLINE Mat44f_Out RotateAround(Vec3f_In origin, Mat44f_In rotatorsMatrix, Mat44f_In rotationMatrix)
{
...
FORCEINLINE Mat44f_Out MakePerspectiveMatrix(float fFOV, float fAspectRatio, float fNearClip, float fFarClip)
{
...
FORCEINLINE Mat44f_Out MakeOrthographicMatrix(float fWidth, float fHeight, float fNear, float fFar)
{
...
FORCEINLINE Mat44f_Out MakeTextureMatrixOffset(u32 unWidth, u32 unHeight)
{
...
FORCEINLINE float CalculateGaussianWeight(s32 nOffset, float fSigma)
{
...
#if SSE_AVAILABLE
FORCEINLINE ScalarV_Out Lerp(ScalarV_In lhs, ScalarV_In rhs, ScalarV_In vLambda)
{
	return lhs + (rhs - lhs) * vLambda;
}

FORCEINLINE Vec2V_Out Lerp(Vec2V_In lhs, Vec2V_In rhs, ScalarV_In vLambda)
{
	return lhs + (rhs - lhs) * vLambda;
}

FORCEINLINE Vec3V_Out Lerp(Vec3V_In lhs, Vec3V_In rhs, ScalarV_In vLambda)
{
	return lhs + (rhs - lhs) * vLambda;
}

FORCEINLINE Vec4V_Out Lerp(Vec4V_In lhs, Vec4V_In rhs, ScalarV_In vLambda)
{
	return lhs + (rhs - lhs) * vLambda;
}

FORCEINLINE ScalarV_Out Abs(ScalarV_In vScalar)
{
	return ScalarV(VectorAbs(vScalar.GetVector()));
}

FORCEINLINE Vec2V_Out Abs(Vec2V_In vVector)
{
	return Vec2V(VectorAbs(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Abs(Vec3V_In vVector)
{
	return Vec3V(VectorAbs(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Abs(Vec4V_In vVector)
{
	return Vec4V(VectorAbs(vVector.GetVector()));
}

template<VecElem splat>
FORCEINLINE Vec2V_Out Splat(Vec2V_In lhs)
{
	return Permute<splat, splat>(lhs);
}

template<VecElem splat>
FORCEINLINE Vec3V_Out Splat(Vec3V_In lhs)
{
	return Permute<splat, splat, splat>(lhs);
}

template<VecElem splat>
FORCEINLINE Vec4V_Out Splat(Vec4V_In lhs)
{
	return Permute<splat, splat, splat, splat>(lhs);
}

template<VecElem pX, VecElem pY>
FORCEINLINE Vec2V_Out Permute(Vec2V_In lhs)
{
	CompileTimeAssert(	(pX >= VecElem::X && pX <= VecElem::Y) &&
						(pY >= VecElem::X && pY <= VecElem::Y), "Invalid Permute Indices! Indices must be between VecElem::X <-> VecElem::Y!");
	CompileTimeAssert(	!(pX == VecElem::X && pY == VecElem::Y), "Invalid Permute Indices! Vector Will Not Change, So Don't Bother Calling Permute!");
...
template<VecElem pX, VecElem pY, VecElem pZ>
FORCEINLINE Vec3V_Out Permute(Vec3V_In lhs)
{
	CompileTimeAssert(	(pX >= VecElem::X && pX <= VecElem::Z) &&
						(pY >= VecElem::X && pY <= VecElem::Z) &&
						(pZ >= VecElem::X && pY <= VecElem::Z), "Invalid Permute Indices! Indices must be between VecElem::X <-> VecElem::Z!");
...
template<VecElem pX, VecElem pY, VecElem pZ, VecElem pW>
FORCEINLINE Vec4V_Out Permute(Vec4V_In lhs)
{
	CompileTimeAssert(	(pX >= VecElem::X && pX <= VecElem::W) &&
						(pY >= VecElem::X && pY <= VecElem::W) &&
						(pZ >= VecElem::X && pY <= VecElem::W) &&
...
template<VecElem pX, VecElem pY>
FORCEINLINE Vec2V_Out Permute(Vec2V_In lhs, Vec2V_In rhs)
{
	CompileTimeAssert(	((pX >= VecElem::X1 && pX <= VecElem::Y1) || (pX >= VecElem::X2 && pX <= VecElem::Y2)) &&
...
template<VecElem pX, VecElem pY, VecElem pZ>
FORCEINLINE Vec3V_Out Permute(Vec3V_In lhs, Vec3V_In rhs)
{
	CompileTimeAssert(	((pX >= VecElem::X1 && pX <= VecElem::Z1) || (pX >= VecElem::X2 && pX <= VecElem::Z2)) &&
						((pY >= VecElem::X1 && pY <= VecElem::Z1) || (pY >= VecElem::X2 && pY <= VecElem::Z2)) &&
...
template<VecElem pX, VecElem pY, VecElem pZ, VecElem pW>
FORCEINLINE Vec4V_Out Permute(Vec4V_In lhs, Vec4V_In rhs)
{
	CompileTimeAssert(	((pX >= VecElem::X1 && pX <= VecElem::W1) || (pX >= VecElem::X2 && pX <= VecElem::W2)) &&
						((pY >= VecElem::X1 && pY <= VecElem::W1) || (pY >= VecElem::X2 && pY <= VecElem::W2)) &&
						((pZ >= VecElem::X1 && pZ <= VecElem::W1) || (pZ >= VecElem::X2 && pZ <= VecElem::W2)) &&
...
FORCEINLINE s32 SignMask(Vec2V_In vVector)
{
	return VectorSignMask(vVector.GetVector()) & 0x3;
}

FORCEINLINE s32 SignMask(Vec3V_In vVector)
{
	return VectorSignMask(vVector.GetVector()) & 0x7;
}

FORCEINLINE s32 SignMask(Vec4V_In vVector)
{
	return VectorSignMask(vVector.GetVector()) & 0xF;
}

FORCEINLINE Vec2V_Out SelectTF(Vec2V_In lhs, Vec2V_In rhs, Vec2V_In condition)
{
	return Vec2V(VectorSelectTF(lhs.GetVector(), rhs.GetVector(), condition.GetVector()));
}

FORCEINLINE Vec3V_Out SelectTF(Vec3V_In lhs, Vec3V_In rhs, Vec3V_In condition)
{
	return Vec3V(VectorSelectTF(lhs.GetVector(), rhs.GetVector(), condition.GetVector()));
}

FORCEINLINE Vec4V_Out SelectTF(Vec4V_In lhs, Vec4V_In rhs, Vec4V_In condition)
{
	return Vec4V(VectorSelectTF(lhs.GetVector(), rhs.GetVector(), condition.GetVector()));
}

template<VecElem elem> 
FORCEINLINE ScalarV_Out ScalarVFromElement(Vector_In vVector)
{
...
template<VecElem elem> 
FORCEINLINE ScalarV_Out ScalarVFromElement(Vec2V_In vVector)
{
	return ScalarVFromElement<elem>(vVector.GetVector());
}

template<VecElem elem> 
FORCEINLINE ScalarV_Out ScalarVFromElement(Vec3V_In vVector)
{
	return ScalarVFromElement<elem>(vVector.GetVector());
}

template<VecElem elem> 
FORCEINLINE ScalarV_Out ScalarVFromElement(Vec4V_In vVector)
{
	return ScalarVFromElement<elem>(vVector.GetVector());
}
...
FORCEINLINE ScalarV_Out Sin(ScalarV_In vRadians)
{
	return ScalarV(VectorSin(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out Sin(Vec2V_In vRadians)
{
	return Vec2V(VectorSin(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out Sin(Vec3V_In vRadians)
{
	return Vec3V(VectorSin(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out Sin(Vec4V_In vRadians)
{
	return Vec4V(VectorSin(vRadians.GetVector()));
}

FORCEINLINE ScalarV_Out ASin(ScalarV_In vRadians)
{
	return ScalarV(VectorASin(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out ASin(Vec2V_In vRadians)
{
	return Vec2V(VectorASin(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out ASin(Vec3V_In vRadians)
{
	return Vec3V(VectorASin(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out ASin(Vec4V_In vRadians)
{
	return Vec4V(VectorASin(vRadians.GetVector()));
}

FORCEINLINE ScalarV_Out Cos(ScalarV_In vRadians)
{
	return ScalarV(VectorCos(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out Cos(Vec2V_In vRadians)
{
	return Vec2V(VectorCos(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out Cos(Vec3V_In vRadians)
{
	return Vec3V(VectorCos(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out Cos(Vec4V_In vRadians)
{
	return Vec4V(VectorCos(vRadians.GetVector()));
}

FORCEINLINE ScalarV_Out ACos(ScalarV_In vRadians)
{
	return ScalarV(VectorACos(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out ACos(Vec2V_In vRadians)
{
	return Vec2V(VectorACos(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out ACos(Vec3V_In vRadians)
{
	return Vec3V(VectorACos(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out ACos(Vec4V_In vRadians)
{
	return Vec4V(VectorACos(vRadians.GetVector()));
}

FORCEINLINE ScalarV_Out Tan(ScalarV_In vRadians)
{
	return ScalarV(VectorTan(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out Tan(Vec2V_In vRadians)
{
	return Vec2V(VectorTan(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out Tan(Vec3V_In vRadians)
{
	return Vec3V(VectorTan(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out Tan(Vec4V_In vRadians)
{
	return Vec4V(VectorTan(vRadians.GetVector()));
}

FORCEINLINE ScalarV_Out ATan(ScalarV_In vRadians)
{
	return ScalarV(VectorATan(vRadians.GetVector()));
}

FORCEINLINE Vec2V_Out ATan(Vec2V_In vRadians)
{
	return Vec2V(VectorATan(vRadians.GetVector()));
}

FORCEINLINE Vec3V_Out ATan(Vec3V_In vRadians)
{
	return Vec3V(VectorATan(vRadians.GetVector()));
}

FORCEINLINE Vec4V_Out ATan(Vec4V_In vRadians)
{
	return Vec4V(VectorATan(vRadians.GetVector()));
}

// Returns: Vec2V(Sin(), Cos(), Sin(), Cos());
FORCEINLINE Vec2V_Out SinCos(ScalarV_In vRadians)
{
...
FORCEINLINE ScalarV_Out Sqrt(ScalarV_In vScalar)
{
	return ScalarV(VectorSqrt(vScalar.GetVector()));
}

FORCEINLINE Vec2V_Out Sqrt(Vec2V_In vVector)
{
	return Vec2V(VectorSqrt(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Sqrt(Vec3V_In vVector)
{
	return Vec3V(VectorSqrt(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Sqrt(Vec4V_In vVector)
{
	return Vec4V(VectorSqrt(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out SqrtSafe(ScalarV_In vScalar, ScalarV_In safeVec /*= ScalarV(I_ZERO)*/)
{
...
FORCEINLINE Vec2V_Out SqrtSafe(Vec2V_In vVector, Vec2V_In safeVec /*= Vec2V(I_ZERO)*/)
{
...
FORCEINLINE Vec3V_Out SqrtSafe(Vec3V_In vVector, Vec3V_In safeVec /*= Vec3V(I_ZERO)*/)
{
	Vector NotZeroVecMask = VectorIsNotEqual(vVector.GetVector(), VectorSet(0.0f));
	Vector sqrtVal = VectorSqrt(vVector.GetVector());
...
FORCEINLINE Vec4V_Out SqrtSafe(Vec4V_In vVector, Vec4V_In safeVec /*= Vec4V(I_ZERO)*/)
{
	Vector NotZeroVecMask = VectorIsNotEqual(vVector.GetVector(), VectorSet(0.0f));
	Vector sqrtVal = VectorSqrt(vVector.GetVector());
...
FORCEINLINE ScalarV Dot(Vec2V_In vVectorA, Vec2V_In vVectorB)
{
	Vector tmp = VectorMultiply(vVectorA.GetVector(), vVectorB.GetVector());
	return ScalarVFromElement<VecElem::X>(VectorHAdd(tmp, tmp));
}

FORCEINLINE ScalarV Dot(Vec3V_In vVectorA, Vec3V_In vVectorB)
{
...
FORCEINLINE ScalarV Dot(Vec4V_In vVectorA, Vec4V_In vVectorB)
{
	Vector tmp = VectorMultiply(vVectorA.GetVector(), vVectorB.GetVector());
	tmp = VectorHAdd(tmp, tmp);
	return ScalarVFromElement<VecElem::X>(VectorHAdd(tmp, tmp));
...
FORCEINLINE ScalarV Mag(Vec2V_In vVector)
{
	return Sqrt(MagSq(vVector));
}

FORCEINLINE ScalarV Mag(Vec3V_In vVector)
{
	return Sqrt(MagSq(vVector));
}

FORCEINLINE ScalarV Mag(Vec4V_In vVector)
{
	return Sqrt(MagSq(vVector));
}

FORCEINLINE ScalarV Length(Vec2V_In vVector)
{
	return Sqrt(LengthSq(vVector));
}

FORCEINLINE ScalarV Length(Vec3V_In vVector)
{
	return Sqrt(LengthSq(vVector));
}

FORCEINLINE ScalarV Length(Vec4V_In vVector)
{
	return Sqrt(LengthSq(vVector));
}

FORCEINLINE ScalarV MagSq(Vec2V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE ScalarV MagSq(Vec3V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE ScalarV MagSq(Vec4V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE ScalarV LengthSq(Vec2V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE ScalarV LengthSq(Vec3V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE ScalarV LengthSq(Vec4V_In vVector)
{
	return Dot(vVector, vVector);
}

FORCEINLINE Vec2V_Out Normalize(Vec2V_In vVector)
{
	Vec2V tmp(vVector);
	return tmp.Normalize();
}

FORCEINLINE Vec3V_Out Normalize(Vec3V_In vVector)
{
	Vec3V tmp(vVector);
	return tmp.Normalize();
}

FORCEINLINE Vec4V_Out Normalize(Vec4V_In vVector)
{
	Vec4V tmp(vVector);
	return tmp.Normalize();
}

//FORCEINLINE Mat33V_Out Normalize(Mat33V_In mMatrix)
//{
//	Mat33V tmp(mMatrix);
//	tmp.Normalize();
//	return tmp;
...
FORCEINLINE Mat44V_Out Normalize(Mat44V_In mMatrix)
{
	Mat44V tmp(mMatrix);
	tmp.Normalize();
	return tmp;
...
FORCEINLINE ScalarV_Out AddInt(ScalarV_In lhs, ScalarV_In rhs)
{
	return ScalarV(VectorAddInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE Vec2V_Out AddInt(Vec2V_In lhs, Vec2V_In rhs)
{
	return Vec2V(VectorAddInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec3V_Out AddInt(Vec3V_In lhs, Vec3V_In rhs)
{
	return Vec3V(VectorAddInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec4V_Out AddInt(Vec4V_In lhs, Vec4V_In rhs)
{
	return Vec4V(VectorAddInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE ScalarV_Out SubtractInt(ScalarV_In lhs, ScalarV_In rhs)
{
	return ScalarV(VectorSubtractInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE Vec2V_Out SubtractInt(Vec2V_In lhs, Vec2V_In rhs)
{
	return Vec2V(VectorSubtractInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec3V_Out SubtractInt(Vec3V_In lhs, Vec3V_In rhs)
{
	return Vec3V(VectorSubtractInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec4V_Out SubtractInt(Vec4V_In lhs, Vec4V_In rhs)
{
	return Vec4V(VectorSubtractInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE ScalarV_Out MultiplyInt(ScalarV_In lhs, ScalarV_In rhs)
{
	return ScalarV(VectorMultiplyInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE Vec2V_Out MultiplyInt(Vec2V_In lhs, Vec2V_In rhs)
{
	return Vec2V(VectorMultiplyInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec3V_Out MultiplyInt(Vec3V_In lhs, Vec3V_In rhs)
{
	return Vec3V(VectorMultiplyInt(lhs.GetVector(), rhs.GetVector()));
}
FORCEINLINE Vec4V_Out MultiplyInt(Vec4V_In lhs, Vec4V_In rhs)
{
	return Vec4V(VectorMultiplyInt(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE ScalarV_Out IntToFloat(ScalarV_In vVector)
{
	return ScalarV(VectorIntToFloat(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out IntToFloat(Vec2V_In vVector)
{
	return Vec2V(VectorIntToFloat(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out IntToFloat(Vec3V_In vVector)
{
	return Vec3V(VectorIntToFloat(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out IntToFloat(Vec4V_In vVector)
{
	return Vec4V(VectorIntToFloat(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out FloatToInt(ScalarV_In vVector)
{
	return ScalarV(VectorFloatToInt(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out FloatToInt(Vec2V_In vVector)
{
	return Vec2V(VectorFloatToInt(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out FloatToInt(Vec3V_In vVector)
{
	return Vec3V(VectorFloatToInt(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out FloatToInt(Vec4V_In vVector)
{
	return Vec4V(VectorFloatToInt(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out Floor(ScalarV_In vVector)
{
	return ScalarV(VectorFloor(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out Floor(Vec2V_In vVector)
{
	return Vec2V(VectorFloor(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Floor(Vec3V_In vVector)
{
	return Vec3V(VectorFloor(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Floor(Vec4V_In vVector)
{
	return Vec4V(VectorFloor(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out Ceil(ScalarV_In vVector)
{
	return ScalarV(VectorCeil(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out Ceil(Vec2V_In vVector)
{
	return Vec2V(VectorCeil(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Ceil(Vec3V_In vVector)
{
	return Vec3V(VectorCeil(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Ceil(Vec4V_In vVector)
{
	return Vec4V(VectorCeil(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out Trunc(ScalarV_In vVector)
{
	return ScalarV(VectorTrunc(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out Trunc(Vec2V_In vVector)
{
	return Vec2V(VectorTrunc(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Trunc(Vec3V_In vVector)
{
	return Vec3V(VectorTrunc(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Trunc(Vec4V_In vVector)
{
	return Vec4V(VectorTrunc(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out Round(ScalarV_In vVector)
{
	return ScalarV(VectorRound(vVector.GetVector()));
}

FORCEINLINE Vec2V_Out Round(Vec2V_In vVector)
{
	return Vec2V(VectorRound(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Round(Vec3V_In vVector)
{
	return Vec3V(VectorRound(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Round(Vec4V_In vVector)
{
	return Vec4V(VectorRound(vVector.GetVector()));
}

FORCEINLINE ScalarV_Out Log2(ScalarV_In vScalar)
{
	return ScalarV(VectorLog2(vScalar.GetVector()));
}
FORCEINLINE Vec2V_Out Log2(Vec2V_In vVector)
{
	return Vec2V(VectorLog2(vVector.GetVector()));
}

FORCEINLINE Vec3V_Out Log2(Vec3V_In vVector)
{
	return Vec3V(VectorLog2(vVector.GetVector()));
}

FORCEINLINE Vec4V_Out Log2(Vec4V_In vVector)
{
...
FORCEINLINE Vec3V_Out Cross(Vec3V_In lhs, Vec3V_In rhs)
{
	return Vec3V(VectorCrossProduct(lhs.GetVector(), rhs.GetVector()));
}

FORCEINLINE Mat44V Mat34ToMat44(Mat34V_In mMatrix)
{
...
FORCEINLINE Mat34V Mat44ToMat34(Mat44V_In mMatrix)
{
...
FORCEINLINE Mat44V_Out MatrixInverse(Mat44V_In mMatrix)
{
	/*
		Derived From The Intel Paper At:
		ftp://download.intel.com/design/pentiumiii/sml/24504301.pdf
...
FORCEINLINE Mat44V_Out Transpose(Mat44V_In mMatrix)
{
...
FORCEINLINE Mat44V_Out Transpose3x3(Mat44V_In mMatrix)
{
	Mat44V retVal(mMatrix);
	retVal.Transpose3x3();
	return retVal;
...
FORCEINLINE Mat44V_Out RotateAround(Vec3V_In origin, Mat44V_In rotatorsMatrix, Mat44V_In rotationMatrix)
{
...
FORCEINLINE Mat44V_Out MakePerspectiveMatrixV(float fFOV, float fAspectRatio, float fNearClip, float fFarClip)
{
	float yScale = 1 / tan(fFOV * 0.5f);
	float xScale = yScale / fAspectRatio;
...
FORCEINLINE Mat44V_Out MakeOrthographicMatrixV(float fWidth, float fHeight, float fNear, float fFar)
{
...
FORCEINLINE Mat44V_Out MakeTextureMatrixOffsetV(unsigned int unWidth, unsigned int unHeight)
{
...
FORCEINLINE VecCmpResult::VecCmpResult(const bool& bX)
{
	m_VectorMask = Vec4V(VectorIsNotEqualInt(VectorSet(0), VectorSet(bX, 0, 0, 0)));
}
FORCEINLINE VecCmpResult::VecCmpResult(const bool& bX, const bool& bY)
{
	m_VectorMask = Vec4V(VectorIsNotEqualInt(VectorSet(0), VectorSet(bX, bY, 0, 0)));
}
FORCEINLINE VecCmpResult::VecCmpResult(const bool& bX, const bool& bY, const bool& bZ)
{
	m_VectorMask = Vec4V(VectorIsNotEqualInt(VectorSet(0), VectorSet(bX, bY, bZ, 0)));
}
FORCEINLINE VecCmpResult::VecCmpResult(const bool& bX, const bool& bY, const bool& bZ, const bool bW)
{
	m_VectorMask = Vec4V(VectorIsNotEqualInt(VectorSet(0), VectorSet(bX, bY, bZ, bW)));
}

#if SSE_AVAILABLE
FORCEINLINE VecCmpResult::VecCmpResult(Vector_In result) : m_VectorMask(result) {}
#endif // SSE_AVAILABLE

FORCEINLINE VecCmpResult::operator Vec2f() const { return *reinterpret_cast<const Vec2f*>(&m_VectorMask); }
FORCEINLINE VecCmpResult::operator Vec3f() const { return *reinterpret_cast<const Vec3f*>(&m_VectorMask); }
FORCEINLINE VecCmpResult::operator Vec4f() const { return *reinterpret_cast<const Vec4f*>(&m_VectorMask); }

#if SSE_AVAILABLE
FORCEINLINE VecCmpResult::operator Vec2V() const { return Vec2V(m_VectorMask.GetVector()); }
FORCEINLINE VecCmpResult::operator Vec3V() const { return Vec3V(m_VectorMask.GetVector()); }
FORCEINLINE VecCmpResult::operator Vec4V() const { return Vec4V(m_VectorMask.GetVector()); }
FORCEINLINE VecCmpResult::operator Vector() const { return m_VectorMask.GetVector(); }
#endif // SSE_AVAILABLE

FORCEINLINE int VecCmpResult::GetResultMask() const { return SignMask(m_VectorMask); }
FORCEINLINE VecCmpResult::operator s32 () const { return GetResultMask(); }

FORCEINLINE bool VecCmpResult::IsTrueAny() const { return GetResultMask() != 0; }
FORCEINLINE bool VecCmpResult::IsTrueAll() const { return (GetResultMask() & ((1 << (int)VecElem::X) | (1 << (int)VecElem::Y) | (1 << (int)VecElem::Z) | (1 << (int)VecElem::W))) != 0; }

template<VecElem index>
FORCEINLINE bool VecCmpResult::IsTrue() const { CompileTimeAssert(index > = VecElem::X && index <= VecElem::W, "Invalid VecCmpResult Index!"); return (GetResultMask() & (1 << index)) != 0; }

template<VecElem index0, VecElem index1>
FORCEINLINE bool VecCmpResult::IsTrue() const
{
